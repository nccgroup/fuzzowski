...
class MockHTTPFuzzer(IFuzzer):
    ...
    def define_nodes(*args, **kwargs) -> None:
        ...
        with s_block('post_body'):
            s_string(b'op', name='param_op_name', fuzzable=False)
            s_delim(b'=', name='param_op_delim', fuzzable=False)
            s_string(b'fuzz', name='param_op_value', fuzzable=False)
        ...
        s_static(b'Token: ')
        s_variable('token', value=b'NOTSET')
        s_static(b"\r\nContent-Length: ")
        s_size("post_body", output_format="ascii", signed=True, fuzzable=True, name='ContentLength_size')
        s_static(b'\r\n\r\n')
        # body
