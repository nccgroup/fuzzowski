...
class Request(IFuzzable):
    ...
    def original_value(self):
        ...
        if self.block_stack:
            raise exception.FuzzowskiRuntimeError("UNCLOSED BLOCK: %s" % self.block_stack[-1].name)
    ...
    def pop(self):
        ...
        if not self.block_stack:
            raise exception.FuzzowskiRuntimeError("BLOCK STACK OUT OF SYNC")
    ...
    def push(self, item):
        ...
        if hasattr(item, "name") and item.name:
            ...
            if item.name in self.names.keys():
                raise exception.FuzzowskiRuntimeError("BLOCK NAME ALREADY EXISTS: %s" % item.name)
    ...
    def render(self, replace_node=None, replace_value=None, original=False):
        ...
        for item in self.stack:
            a = item.render(replace_node=replace_node, replace_value=replace_value, original=original)
            self._rendered += a
