...
# from .sessions import Session, open_test_run
from .sessions import Session, Connection
...
def s_get(name=None):
    ...
    if name not in blocks.REQUESTS:
        raise exception.FuzzowskiRuntimeError("blocks.REQUESTS NOT FOUND: %s" % name)
...
def s_initialize(name):
    ...
    if name in blocks.REQUESTS:
        raise exception.FuzzowskiRuntimeError("blocks.REQUESTS ALREADY EXISTS: %s" % name)
...
def s_checksum(block_name, algorithm="crc32", length=0, endian=LITTLE_ENDIAN, fuzzable=True, name=None,
    ...
    if block_name in blocks.CURRENT.block_stack:
        raise exception.FuzzowskiRuntimeError("CAN N0T ADD A CHECKSUM FOR A BLOCK CURRENTLY IN THE STACK")
...
def s_size(block_name, offset=0, length=4, endian=LITTLE_ENDIAN, output_format="binary", inclusive=False, signed=False,
    ...
    if block_name in blocks.CURRENT.block_stack:
        raise exception.FuzzowskiRuntimeError("CAN NOT ADD A SIZE FOR A BLOCK CURRENTLY IN THE STACK")
...
def s_update(name, value):
    ...
    if name not in map(lambda o: o.name, blocks.CURRENT.walk()):
        raise exception.FuzzowskiRuntimeError("NO OBJECT WITH NAME '%s' FOUND IN CURRENT REQUEST" % name)
...
def s_lego(lego_type, value=None, options=()):
    ...
    if lego_type not in legos.BIN:
        raise exception.FuzzowskiRuntimeError("INVALID LEGO TYPE SPECIFIED: %s" % lego_type)
...
def s_string(value, size=-1, padding="\x00", encoding="utf-8", fuzzable=True, max_len=0, name=None,
    ...
    s = primitives.String(value, size=size, padding=padding, encoding=encoding, fuzzable=fuzzable, max_len=max_len,
                          name=name, callback_addr=callback_addr, filename=filename, mutations=mutations)
